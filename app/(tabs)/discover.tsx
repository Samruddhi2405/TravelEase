...
const Discover = () => {
  ...
  const fetchPlaceImage = async (placeName: string) => {
    try {
      ...
      if (data.features && data.features[0] && data.features[0].properties.datasource?.raw?.photo_reference) {
        return data.features[0].properties.datasource.raw.photo_reference;
      }
